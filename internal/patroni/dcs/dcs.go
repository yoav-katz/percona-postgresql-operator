...
// recording in one place: the controller.
package dcs
...
type StateCleanup struct {
	...
	// Apply, when non-nil, is server-side applied by the caller with cluster
	...
	// Warning, when non-nil, is recorded as a Warning event on the cluster.
	Warning *Warning
}

// Warning is an event a backend wants recorded on the cluster.
type Warning struct {
