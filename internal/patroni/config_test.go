...
// kubernetesDCSYAML mirrors what the Kubernetes Endpoints DCS backend
// contributes to the cluster-wide config. It is reproduced here rather than
// imported from internal/patroni/dcs, because that package depends on this
// one; importing it back would be a cycle. What the backend actually produces
// is covered by its own tests.
func kubernetesDCSYAML(cluster *v1beta1.PostgresCluster) map[string]any {
...
func TestClusterYAML(t *testing.T) {
	...
	t.Run("PG version defaulted", func(t *testing.T) {
		...
		data, err := clusterYAML(cluster, postgres.HBAs{}, postgres.Parameters{}, kubernetesDCSYAML(cluster))
		assert.NilError(t, err)
