...
// this works identically regardless of DCS backend, since it is
// PostgreSQL/Patroni introspection rather than a DCS read.
//
// host must match a DNS SAN on Patroni's REST API certificate (i.e. the pod's
// own stable DNS name) since Patroni does not certify "localhost".
...
// PodRequiresRestart returns whether PostgreSQL inside the pod has (pending)
// parameter changes that require a PostgreSQL restart, by asking Patroni's own
// monitoring REST API directly. Unlike the package-level PodRequiresRestart
// function, this does not depend on Patroni having written a Kubernetes
// annotation, so it works under any DCS backend.
